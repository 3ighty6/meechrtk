(() => {
  ...
  const similarity = (a,b) => { const A=new Set(words(a)), B=new Set(words(b)); if(!A.size||!B.size)return 0; let n=0; for(const x of A) if(B.has(x))n++; return n/(A.size+B.size-n); };
  function optimize(text, mode='balanced') {
    const lines=text.split(/\r?\n/), out=[], seen=[]; let removed=0;
    const threshold=mode==='aggressive'?0.82:mode==='conservative'?0.96:0.90;
    for(const line of lines){
      ...
      if(!important && seen.some(x => similarity(t,x) >= threshold)){ removed++; continue; }
      seen.push(t); out.push(line);
    }
  ...
  window.addEventListener('message', e => {
    if(e.source!==window || e.data?.source!=='meechrtk') return;
    const r=optimize(String(e.data.text||''), e.data.mode||'balanced');
    window.postMessage({source:'meechrtk-result', ...r}, '*');
  });
})();
