(() => {
  ...
  function hostFor(el){return el.closest('form')||el.parentElement||document.body;}
  function ensureBar(el){
    if(current===el&&bar)return;
    if(bar)bar.remove();current=el;
    bar=document.createElement('div');bar.dataset.meechrtk='1';
    Object.assign(bar.style,{position:'fixed',zIndex:2147483647,display:'flex',gap:'6px',alignItems:'center',padding:'5px 7px',border:'1px solid rgba(128,128,128,.35)',borderRadius:'10px',background:'rgba(20,20,24,.94)',color:'#fff',font:'12px system-ui',boxShadow:'0 4px 18px rgba(0,0,0,.25)'});
    ...
    [optimizeBtn,restoreBtn].forEach(b=>Object.assign(b.style,{border:'0',borderRadius:'7px',padding:'5px 8px',cursor:'pointer'}));
    optimizeBtn.onclick=()=>{const original=getText(el);if(!original.trim())return;lastOriginal=original;const r=optimize(original,select.value);setText(el,r.text);const pct=original.length?((1-r.text.length/original.length)*100):0;stat.textContent=`${pct.toFixed(1)}% shorter • ${r.removed} lines`};
    restoreBtn.onclick=()=>{if(lastOriginal){setText(el,lastOriginal);stat.textContent='Restored'}};
    bar.append(select,optimizeBtn,restoreBtn,stat);document.body.appendChild(bar);
    const position=()=>{const r=el.getBoundingClientRect();bar.style.left=Math.max(8,r.left)+'px';bar.style.top=Math.max(8,r.top-48)+'px'};position();window.addEventListener('scroll',position,{passive:true});window.addEventListener('resize',position);
  }
