...
class Executor:
    ...
    def _cloud(self,p,key,url,prompt,max_tokens,reasoning,model):
        if p=='claude':
            ...
            if reasoning in ('low','medium','high'):
                body['thinking']={'type':'enabled','budget_tokens':{'low':1024,'medium':4096,'high':8192}[reasoning]}
    ...
    def _request(self,url,headers,body,p):
        ...
        try:
            ...
            raise ProviderError(f'{p} HTTP {e.code}: {detail[:1000]}')
        except Exception as e: raise ProviderError(f'{p}: {e}')
        text=self._extract(raw,p);usage=self._usage(raw,p)
        return {'text':text,'model':raw.get('model'),'usage':usage,'raw_id':raw.get('id'),'http_status':status}
    def _extract(self,r,p):
        ...
        if p in ('openai','xai'):
            ...
            return ''
        if p=='google':return ''.join(p.get('text','') for p in r.get('candidates',[{}])[0].get('content',{}).get('parts',[]))
        return (r.get('choices') or [{}])[0].get('message',{}).get('content','')
    def _usage(self,r,p):
        u=r.get('usage',{}) or {}
        return {'input_tokens':u.get('input_tokens',u.get('prompt_tokens',u.get('promptTokenCount',0))), 'output_tokens':u.get('output_tokens',u.get('completion_tokens',u.get('candidatesTokenCount',0))), 'total_tokens':u.get('total_tokens',u.get('totalTokenCount',0))}
