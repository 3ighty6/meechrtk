 ...
 def do_POST(self):
  ...
  try:
   ...
   if p=="/v1/optimize":
    ...
    provider=PROVIDER_ALIASES.get(str(data.get("provider","auto")).lower(),str(data.get("provider","auto")).lower())
    policy=POLICY.classify(request,provider,bool(data.get("stable_prefix")));budget=data.get("budget",policy.context_mode)
   ...
   if p=="/v1/route":
    request=str(data.get("request","")).strip()
    if not request:return self._send(400,{"ok":False,"error":"request is required"})
    preferred=PROVIDER_ALIASES.get(str(data.get("preferred","")).lower(),data.get("preferred")) if data.get("preferred") else None
   ...
   if p=="/v1/capacity/configure":
    provider=PROVIDER_ALIASES.get(str(data.get("provider","")).strip().lower(),str(data.get("provider","")).strip().lower())
   ...
   if p=="/v1/capacity/usage":
    provider=PROVIDER_ALIASES.get(str(data.get("provider","")).strip().lower(),str(data.get("provider","")).strip().lower());tokens=int(data.get("tokens",0));CAPACITY.record_usage(provider,tokens,float(data.get("cost",0)),data.get("latency_ms"));return self._send(200,{"ok":True,"provider":asdict(CAPACITY.states[provider])})
   if p=="/v1/memory":
