...
class CapacityManager:
    ...
    It never fabricates quota or bypasses provider limits. Unknown quotas remain None.
    """
    def __init__(self):
    ...
    def route(self,task,required_tokens=0,preferred=None,local_ok=True,budget_usd=None):
        ...
        for name,s in self.states.items():
            if name=='local' and not local_ok:continue
            ...
            score=0.0
            score += {'low':1.0,'medium':2.0,'high':3.0}[complexity] if name not in ('ollama','lmstudio','local') else {'low':3.0,'medium':1.5,'high':.5}[complexity]
        ...
        if not candidates:return {'ok':False,'error':'No provider has sufficient declared capacity','complexity':complexity}
    ...
    def _reason(name,s,complexity):
        ...
        if name in ('ollama','lmstudio','local'):bits.append('local/low-cost route')
